...
from lib.io_helper import save_args, save_backproject_obj, save_viewpoints
from lib.mesh_helper import adjust_uv_map, init_mesh
...
def init_args():
    ...
    parser.add_argument("--update_steps", type=int, default=0)
    parser.add_argument(
        "--update_mode",
        type=str,
        default="heuristic",
    ...
    parser.add_argument(
        "--use_principle", action="store_true", help="poperate on multiple objects"
    )
    parser.add_argument(
        "--use_shapenet", action="store_true", help="operate on ShapeNet objects"
    )
    parser.add_argument(
        "--use_objaverse", action="store_true", help="operate on Objaverse objects"
    )
    parser.add_argument(
        "--use_scene", action="store_true", help="operate on scene objects"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--post_process", action="store_true", help="post processing the texture"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="x coordinate of camera center",
    )
    parser.add_argument(
        "--camera_center_y",
        type=float,
        default=0.0,
        help="y coordinate of camera center",
    )
    parser.add_argument(
        "--camera_center_z",
        type=float,
        default=0.0,
        help="z coordinate of camera center",
    )
    ...
    if args.device == "a6000":
        ...
        setattr(args, "fragment_k", 1)
        setattr(args, "image_size", 768)
        setattr(args, "uv_size", 4096)  # TODO set to 4096
...
if __name__ == "__main__":
    ...
    mesh, _, faces, aux, principle_directions, mesh_center, mesh_scale = init_mesh(
        ...
        mesh_center=torch.tensor(
            [args.camera_center_x, args.camera_center_y, args.camera_center_z]
        ),
    )
    ...
    (dist_list, elev_list, azim_list, sector_list, view_punishments) = init_viewpoints(
        ...
        use_principle=args.use_principle,
    ...
    if args.use_shapenet or args.use_objaverse:
        ...
        NUM_PRINCIPLE = 6
    # NUM_PRINCIPLE = (
    #     10 if args.use_shapenet or args.use_objaverse else 6
    # )  # TODO set to 8, 16, or 24
    # if not args.use_principle:
    #     NUM_PRINCIPLE = len(dist_list)
    ...
    for view_idx in range(NUM_PRINCIPLE):
        ...
        if args.no_repaint and view_idx != 0:
            actual_generate_mask_image = Image.fromarray(
                (np.ones_like(np.array(generate_mask_image)) * 255.0).astype(np.uint8)
            )
        else:
        ...
        if (
            ...
            (
                diffused_image,
                diffused_image_before,
                diffused_image_after,
            ) = apply_controlnet_depth(
                controlnet,
                ddim_sampler,
                init_image.convert("RGBA"),
